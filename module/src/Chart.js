...
export default class Chart {
  ...
  removeData(label) {
    ...
    const findLabel = this.#data.find((dataEntry) => dataEntry.label === label);
    if (!findLabel) {
      throw new Error(`Data with label "${label}" not found`);
    } else {
      this.#data = this.#data.filter((dataEntry) => dataEntry.label !== label);
    }
  }
  ...
  updateData(label, newValue) {
    ...
    const dataEntry = this.#data.find((dataEntry) => dataEntry.label === label);
    if (!dataEntry) {
      throw new Error(`Data with label "${label}" not found`);
    } else {
   ...
   */
  getTotalEntries() {
    return this.#data.reduce((total, dataEntry) => total + dataEntry.value, 0);
  }
